...
class BelongsTo extends RelationField
{
    ...
    public function __construct($relationName, $relatedClass, $name = null, $label = null, $attrs = null)
    {
        ...
        $this->endpoint(\PrslV2::getRelationFieldEndpoint($this->name));
    }
    ...
    public function getOptions(Model $record, $filter = null)
    {
        ...
        if ($this->optionHandlerCallback) {
            $collection = call_user_func(
                ...
                $relatedModel::query()->when($filter !== null, fn (Builder $q) => $q->where($this->titleField, 'like', '%'.$filter.'%'))->orderBy($this->titleField),
                $record,
                $this
            );
            ...
            $collection = $relatedModel::query()
                ->when($filter !== null, fn (Builder $q) => $q->where($this->titleField, 'like', '%'.$filter.'%'))
